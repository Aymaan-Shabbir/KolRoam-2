...
const getUser = cache(
    async(username:string, logggedInUserId:string)=>{
        const user = await prisma.user.findFirst({
            ...
            select:getUserDataSelect(logggedInUserId)
        })
        if(!user) notFound();
    ...
    return <div className="h-fit w-full space-y-5 rounded-2xl bg-card p-5 shadow:sm">
        ...
        <div className="flex flex-wrap gap-3 sm:flex-nowrap">
            <div className="me-auto space-y-3">
                ...
                <div className="text-xs text-muted-foreground">Date joined {formatDate(user.createdAt,"MMM d,yyyy")}</div>
